    ...
    async def check_macd(self):
        ...
        for symbol in self.symbols:
            ...
            if((data_macd['MACD'].iloc[-2]>data_macd['Signal'].iloc[-2] and data_macd['Histogram'].iloc[-2]>0) and (data_macd['MACD'].iloc[-1]<data_macd['Signal'].iloc[-1] and data_macd['Histogram'].iloc[-1]<0)):
                results.append(f'{symbol}: SELL (MACD) - Price: {data["close"].iloc[-1]}')
                ...
                results.append(f'{symbol}: BUY (MACD) - Price: {data["close"].iloc[-1]}')
        return results
    ...
    async def check_bollinger_bands(self):
        ...
        for symbol in self.symbols:
            ...
            if data['close'].iloc[-1] > upper.iloc[-1]:
                results.append(f'{symbol}: SELL (Bollinger Bands) - Price: {data["close"].iloc[-1]}')
            elif data['close'].iloc[-1] < lower.iloc[-1]:
                results.append(f'{symbol}: BUY (Bollinger Bands) - Price: {data["close"].iloc[-1]}')
        return results
    ...
    async def check_ema_200(self):
        ...
        for symbol in self.symbols:
            ...
            if data['close'].iloc[-1] > ema200.iloc[-1] and data['close'].iloc[-2] < ema200.iloc[-1]:
                results.append(f'{symbol}: SUPPORT (EMA 200) - Price: {data["close"].iloc[-1]}')
            elif data['close'].iloc[-1] < ema200.iloc[-1] and data['close'].iloc[-2] > ema200.iloc[-1]:
                results.append(f'{symbol}: RESISTANCE (EMA 200) - Price: {data["close"].iloc[-1]}')
        return results
